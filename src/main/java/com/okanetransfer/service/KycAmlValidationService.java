...
public class KycAmlValidationService {
    ...
    /**
     * REQUIRES_NEW : s'exécute dans sa propre transaction.
     * Une exception ici ne marque PAS la transaction parente comme rollback-only.
     */
    ...
    public void validateTransfer(String senderDocNumber, String recipientDocNumber,
        ...
        // 2. Validation du bénéficiaire (optionnelle — recipientDocNumber peut être null)
    ...
    // ─── Validation KYC expéditeur ────────────────────────────────────────────

    private void validateSenderKyc(String documentNumber) {
        // Garde : un CIN expéditeur null est une erreur métier claire
        if (documentNumber == null || documentNumber.isBlank()) {
            throw new KycAmlValidationException("Le CIN/Passeport de l'expéditeur est obligatoire.");
        }
        ...
        if (kycProfile.getKycStatus() == KycStatus.REJECTED) {
            throw new KycAmlValidationException("Client sur liste noire. Transfert interdit.");
        }
    ...
    // ─── Validation KYC bénéficiaire ──────────────────────────────────────────

    private void validateRecipientKyc(String documentNumber) {
        // Le CIN bénéficiaire est OPTIONNEL à l'envoi — on ignore s'il est absent
        if (documentNumber == null || documentNumber.isBlank()) {
            return;
        }
        ...
        if (profile.isPresent() && profile.get().getKycStatus() == KycStatus.REJECTED) {
            throw new KycAmlValidationException("Bénéficiaire sur liste noire. Transfert interdit.");
        }
    }
    ...
    public KycProfile createKycProfile(String documentNumber, String documentType, String fullName,
        ...
        profile.setRiskLevel(assessRiskLevel(monthlyIncome, nationality));
        return kycProfileRepository.save(profile);
    }

    private RiskLevel assessRiskLevel(BigDecimal monthlyIncome, String nationality) {
        if (HIGH_RISK_COUNTRIES.contains(nationality)) return RiskLevel.HIGH;
        if (monthlyIncome != null && monthlyIncome.compareTo(new BigDecimal("50000")) >= 0) return RiskLevel.MEDIUM;
